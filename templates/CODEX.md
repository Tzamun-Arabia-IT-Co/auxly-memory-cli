...
- If using shell (Codex CLI): use `auxly view <file>` to read, `auxly write` to persist.
...
## Reading Memory (Shell — Codex CLI)
```bash
...
## Writing Memory (MCP)
- `auxly_memory_write` with `{"file": "infra.md", "diff": "+- CI/CD: GitHub Actions", "reason": "Detected in .github/workflows"}`

## Writing Memory (Shell — Codex CLI)
```bash
auxly write \
  --agent codex-<session> \
  --provider codex \
...
codex mcp add auxly-memory /usr/local/bin/auxly mcp-server
```
