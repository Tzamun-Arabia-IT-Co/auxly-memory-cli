...
## How Gemini Should Use This Memory
...
## Reading Memory (Shell / Gemini CLI)
```bash
...
## Writing Memory (MCP)
- `auxly_memory_write` with `{"file": "preferences.md", "diff": "+- Style: concise", "reason": "User preference"}`

## Writing Memory (Shell / Gemini CLI)
```bash
auxly write \
  --agent gemini-session-<id> \
  --provider gemini \
  --file <target_file> \
