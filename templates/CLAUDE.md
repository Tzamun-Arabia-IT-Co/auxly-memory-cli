...
## How Claude Should Use This Memory
...
- If using shell (Claude Code): use `auxly view <file>` to read, `auxly write` to persist.
- Always provide a `--reason` explaining why the memory is being updated.

## Reading Memory (MCP — Claude Desktop)
...
## Reading Memory (Shell — Claude Code CLI)
```bash
...
## Writing Memory (MCP — Claude Desktop)
- `auxly_memory_write` with `{"file": "identity.md", "diff": "+- Name: Alex", "reason": "User introduced themselves"}`

## Writing Memory (Shell — Claude Code CLI)
```bash
auxly write \
...
```bash
claude mcp add auxly-memory /usr/local/bin/auxly mcp-server
```
...
## What to Remember
- User preferences (communication style, frameworks, languages)
- Important decisions made during conversation
- Project context that future sessions should know
...
- `/auxly-status`: Call the `auxly_skill_status` tool to show live loopback daemon and ngrok tunnel diagnostics.
